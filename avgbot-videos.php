...
?>
<div class="wrap">
<h1 class="wp-heading-inline">AvgBot Video Importer</h1>
...
$CatUrl = $playerUrl.'/token/'.$token;
	...
	preg_match_all('@<a id="videocats" href="(.*?)">(.*?)</a>@si' , $ShowVidCats , $vidCat);
    $countall=count($vidCat[2]);
	if ($countall>0){
		echo '<br><select name="forma" onchange="location = this.value;"><option value"admin.php?page=avgbotVideos">-- Select A Video Category --</option>';
		for($i=0;$i<$countall;$i++){
			echo'<option value="admin.php?page=avgbotVideos&newUrl='.$vidCat[1][$i].'"> '.$vidCat[2][$i].' </option>';
		}
		echo '</select>';
	}
	echo ' Please Select a category from our '.$countall.' video categories';
...
if(isset($_POST[''.$formid.''])) {
	if (wp_verify_nonce($_POST['_avgnonce'], 'avgbot-video-nonce')) {
	include "avgBotClass.php";
	$avgBot = new avgBotClass();
	$avgBot->title = sanitize_avgbot_array_text($_POST['avgTitle']);
	$metas = array(
		'avgimported'	=> sanitize_avgbot_array_text($_POST['avgVideoHash'])
	);
	$meta_added = array();
	$iframecode = "";
	...
	echo '<div id="message" class="updated notice notice-success is-dismissible"><p><strong>'.sanitize_avgbot_array_text($_POST["avgTitle"]).'</strong> titled video succesfully imported!</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Bu mesajı gizle.</span></button></div>';
}else{
...
<?php
	...
	$ShowVideos = startAvgBot($CatLink);
	preg_match_all('@<div class="catvideolist" style="display:none">
				...
				<videotime>(.*?)</videotime>
				<videotags>(.*?)</videotags>
				</div>@si' , $ShowVideos , $catVid);
    preg_match_all('@<li><a class="vcatpage" href="(.*?)">(.*?)</a>@si' , $ShowVideos , $catVidPage);
	$countall=count($catVid[1]); 
	$countpage=count($catVidPage[1]); 
	if ($countall>0){
	echo '
		<div class="wp-list-table widefat plugin-install">
<h2 class="screen-reader-text">Video List</h2>
<div id="the-list">';
		for($i=0;$i<$countall;$i++){
...
echo '</div><br class="clear">';
		...
		<li class="event-none">
				...
				<span class="msg-info"><strong>#This video was previously imported!</strong> Also you can import it again with a Unique Title...</span>
		</li>
	</ul><br class="clear">';
			 }
			$avgnonce = wp_create_nonce('avgbot-video-nonce');	
			...
			<strong>Original Cat: </strong> '.$catVid[5][$i].'</div>
				...
				<div class="column-compatibility">
					 <input type="hidden" value="addVideo-'.$i.'"  name="addAvgform">
					 <input type="hidden" value="'.$catVid[3][$i].'"  name="avgVideoHash">
					 <input type="hidden" name="_avgnonce" value="'.$avgnonce.'">';	
						 echo'<input type="submit" class="button button-primary button-large" name="addVideo-'.$i.'"  value="Add Video">';
...
<?php
	...
	if ($countpage>0){
		...
		for($i=0;$i<$countpage;$i++){
			echo'<a class="button button-primary" href="admin.php?page=avgbotVideos&newUrl='.$catVidPage[1][$i].'">'.$catVidPage[2][$i].'</a>';
		}
